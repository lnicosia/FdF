...
/*   Updated: 2019/01/30 16:14:10 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fcoord2	pre_pc_project(t_coord3 c, t_env data)
{
	...
	res.x = (((float)c.x - data.map_width / 2) / (-((float)c.z * data.scale.z)
				+ data.map[data.zmax].z + data.zlimit + 1));
	res.y = (((float)c.y - data.map_height / 2) / (-((float)c.z * data.scale.z)
				+ data.map[data.zmax].z + data.zlimit + 1));
	return (res);
}
