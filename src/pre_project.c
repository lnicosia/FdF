...
/*   Updated: 2019/01/28 15:24:43 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fcoord2	pre_pc_project(t_coord3 c, t_env data)
{
	...
	res.x = (((float)c.x - data.map_width / 2) / (-(float)c.z * data.scale.z + data.map[data.zmax].z + 1));
	res.y = (((float)c.y - data.map_height / 2) / (-(float)c.z * data.scale.z + data.map[data.zmax].z +
				1));
	return (res);
}
