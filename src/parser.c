...
/*   Updated: 2023/02/15 17:49:38 by lumenthi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		close_parser(int fd, int height, int width)
{
	if (close(fd) == -1)
		return (CLOSE_ERROR);
	if (width < 2 || height < 2)
...
int		parser(t_list **map, char *file, int *map_height, int *map_width)
{
	...
	return (close_parser(fd, *map_height, *map_width));
}
